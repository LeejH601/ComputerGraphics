...
CAnimationSet::CAnimationSet(float fLength, int nFramesPerSecond, int nKeyFrames, int nAnimatedBones, char* pstrName)
{
	...
	for (int i = 0; i < nKeyFrames; i++) m_ppxmf4x4KeyFrameTransforms[i] = new glm::mat4x4[nAnimatedBones];
}
...
glm::mat4x4 CAnimationSet::GetSRT(int nBone, float fPosition)
{
	...
	for (int i = 0; i < (m_nKeyFrames - 1); i++)
	{
		if ((m_pfKeyFrameTimes[i] <= fPosition) && (fPosition < m_pfKeyFrameTimes[i + 1]))
		{
			...
			xmf4x4Transform = glm::mix(m_ppxmf4x4KeyFrameTransforms[i][nBone], m_ppxmf4x4KeyFrameTransforms[i + 1][nBone], t);
			break;
		}
	}
...
void CAnimationController::AdvanceTime(float fTimeElapsed, CObject* pRootGameObject)
{
